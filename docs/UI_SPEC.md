...
* **중앙 (Center):** 현재가 (천 단위 콤마 포맷팅 적용)
* **우측 (Right):** 24h 변동률 (%) 및 변동폭
    * **색상 규칙:** 상승 시 `Color.Red`, 하락 시 `Color.Blue`, 보합 시 테마 기본 텍스트 색상.
...
* **클릭 이벤트:** 아이템 클릭 시 해당 종목의 `symbol`(예: "KRW-BTC")을 네비게이션 인자로 전달하며 **[2. 호가창 상세 화면]**으로 이동.
